import { Component, OnInit, ViewChildren, Query, QueryList } from '@angular/core';
...
export class DanhSachGheBusComponent implements OnInit {
  ...
  @ViewChildren(GheBusComponent) listGhe: QueryList<GheBusComponent>;
  ...
  danhSachGheDaDat = [];
  HuyChonGhe(ghe) {
    this.danhSachGheDaDat.splice(this.danhSachGheDaDat.lastIndexOf(ghe), 1);
    let change = this.listGhe.find(item => item.ghe.SoGhe === ghe.SoGhe);
    change.trangThaiChon = false;
    console.log(change);
  }
  ...
  daChonGhe(obj) {
    ...
    if (obj.TrangThai === true) {
      ...
      this.danhSachGheDaDat.splice(this.danhSachGheDaDat.lastIndexOf(obj.ghe), 1);
    }
